...
/*   Updated: 2024/06/01 23:36:09 by mel-akhd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void								ft_pixel_put(mlx_image_t *img, int x, int y,
										int color);
void								ft_render_image(t_cub3d *cub);
int									is_inside_screen(t_vec2 *vec);
void								ft_draw_line(t_cub3d *instance,
										const t_vec2 *a, const t_vec2 *b,
										int color);
...
void								draw_square(t_cub3d *cub, t_vec2 pos,
										int size, int color);
void								draw_circle(t_cub3d *cub, t_vec2 pos,
										int radius, int color);
double								degree_to_radian(double angle_degrees);
void								ft_key_hooks(void *param);
float								ray_cast(t_cub3d *cub, t_map *map,
										t_cast_request *request);
int32_t								get_color_texture(mlx_texture_t *txt, int x,
										int y);
void								key_pess_hook(mlx_key_data_t key,
										void *param);
void								fire_mouse_hook(mouse_key_t button,
										action_t action, modifier_key_t mods,
										void *param);
int									parse_map(t_cub3d *cub, int argc,
										char **argv);
...
int									dup_checklist(
										t_duplication_checklist **list,
										char *component);
int									get_image_path(t_cub3d *cub, char **dist,
										char *line);
...
void								solve_colors(t_cub3d *cub, char *line,
										int i, int *solved);
